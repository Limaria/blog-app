...
const NewBlogForm = (props: CategorySelectorProps) => {
  ...
  if (!session && status !=="loading") return(
    <div>You mast be signed in to post</div>
  )
  ...
  if(submitted) return(
    ...
    return(
     <div className="min-h-[calc(100vh-130px)] py-2 container flex flex-col mt-12">
      <form className="flex flex-col flex-1 items-stretch justify-center h-ful text-left" onSubmit={handleSubmit}>
        ...
        <div className="self-start">
          ...
          <label className="text-slate-600 mb-3">{thumbnail ? "Change Image" : "Add thumbnail image(optional)"}</label>
            ...
            <CategoryDropdown selected={categoryID} setSelected={(selected: number) => setCategoryID(selected)} />
        </div>
